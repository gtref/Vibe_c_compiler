...
/**
 * No-op job used by tests to exercise thread-pool queueing; the argument is ignored.
 * @param arg Pointer argument (ignored).
 */
...
/**
 * Exercise thread pool security checks for max threads, shutdown behavior, and queue capacity.
 *
 * Runs a sequence of assertions that:
 * - Creating a pool with more than 1024 threads is rejected.
 * - A normal-sized pool can be created.
 * - Jobs submitted after the pool is marked shutdown are rejected.
 * - The pool enforces a maximum queue size and rejects additional jobs when full.
 * The function signals workers and destroys the pool before reporting test summary.
 *
